...
object AppModule {
    ...
    // Migration from v8 → v9: add piltoverArchiveId to decks
    val MIGRATION_8_9 = object : Migration(8, 9) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE decks ADD COLUMN piltoverArchiveId TEXT")
        }
    }

    val MIGRATION_9_10 = object : Migration(9, 10) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE card_identities ADD COLUMN power INTEGER")
            db.execSQL("ALTER TABLE card_identities ADD COLUMN mightBonus INTEGER")
            db.execSQL("ALTER TABLE card_identities ADD COLUMN maxCopies INTEGER")
            db.execSQL("ALTER TABLE card_identities ADD COLUMN banEffectiveDate TEXT")
        }
    }
